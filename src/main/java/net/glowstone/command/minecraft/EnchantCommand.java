...
public class EnchantCommand extends VanillaCommand
{
	...
	{
		super( "enchant", "Adds an enchantment to the currently by a player held item", "/enchant <player> <enchantment ID> [level]", Collections.emptyList() );
	...
	public boolean execute( CommandSender sender, String label, String[] args )
	{
		...
		players.stream().filter( player -> player.getItemInHand() != null ).filter( player -> player.getItemInHand().getData().getItemType() != Material.AIR ).filter( player -> enchantment.canEnchantItem( player.getItemInHand() ) ).forEach( player -> {
