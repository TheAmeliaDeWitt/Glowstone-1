...
public class TestForBlockCommand extends VanillaCommand
{
	...
	{
		super( "testforblock", "Tests for a certain block at a given location", "/testforblock <x> <y> <z> <block> [dataValue|state] [dataTag]", Collections.emptyList() );
	...
	public boolean execute( CommandSender sender, String label, String[] args )
	{
		...
		{
			sender.sendMessage( ChatColor.RED + "The block at " + location.getBlockX() + ", " + location.getBlockY() + ", " + location.getBlockZ() + " is " + ItemIds.getName( block.getType() ) + " (expected: " + ItemIds.getName( type ) + ")" );
			return false;
		}
		if ( args.length > 4 )
		{
			...
			{
				sender.sendMessage( ChatColor.RED + "The block at " + location.getBlockX() + ", " + location.getBlockY() + ", " + location.getBlockZ() + " had the data value of " + block.getData() + " (expected: " + data + ")" );
				return false;
			}
			else if ( !data.isNumeric() )
			{
				try
				{
					...
					if ( !matches )
					{
						...
						sender.sendMessage( ChatColor.RED + "The block at " + location.getBlockX() + ", " + location.getBlockY() + ", " + location.getBlockZ() + " did not match the expected state of " + state );
						return false;
					}
				}
		...
		if ( args.length > 5 && block.getBlockEntity() != null )
		{
			...
			try
			{
				...
				{
					sender.sendMessage( ChatColor.RED + "The block at " + location.getBlockX() + ", " + location.getBlockY() + ", " + location.getBlockZ() + " did not have the required NBT keys" );
					return false;
				}
			}
