...
public class EffectCommand extends VanillaCommand
{
	...
	{
		super( "effect", "Gives a player an effect", "/effect <player> clear " + "OR /effect <player> <effect> [seconds] [amplifier] [hideParticles]", Collections.emptyList() );
	...
	public boolean execute( CommandSender sender, String label, String[] args )
	{
		...
		else
		{
			...
			for ( GlowPlayer player : players )
			{
				...
				sender.sendMessage( "Given " + effectType.getName() + " (ID " + effectType.getId() + ") * " + amplifier + " to " + player.getName() + " for " + duration / 20 + " seconds" );
			}
			return true;
		}
	}
