...
{
	private static final List<String> SUBCOMMANDS = Arrays.asList( "about", "chunk", "eval", "help", "property", "vm", "world" );
	...
	public boolean execute( CommandSender sender, String label, String[] args )
	{
		...
		if ( "chunk".equalsIgnoreCase( args[0] ) )
		{
			...
			{
				sender.sendMessage( ChatColor.RED + "This command may only be used by physical objects" );
				return false;
			}
		...
		sender.sendMessage( ChatColor.RED + "Usage: /" + label + " <" + SUBCOMMANDS.stream().collect( Collectors.joining( "|" ) ) + ">" );
		return false;
	}
	...
	public List<String> tabComplete( CommandSender sender, String alias, String[] args ) throws IllegalArgumentException
	{
		...
		{
			return StringUtil.copyPartialMatches( args[1], System.getProperties().stringPropertyNames(), new ArrayList<>( System.getProperties().stringPropertyNames().size() ) );
		}
		if ( args.length == 2 && ( args[0].equalsIgnoreCase( "world" ) || args[0].equalsIgnoreCase( "worlds" ) ) && sender instanceof Player )
		{
			...
			return StringUtil.copyPartialMatches( args[1], worlds, new ArrayList<>( worlds.size() ) );
		}
		return Collections.emptyList();
	}
}
