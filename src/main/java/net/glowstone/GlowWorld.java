...
public final class GlowWorld implements World
{
	...
	public GlowWorld( GlowServer server, WorldCreator creator, WorldStorageProvider worldStorageProvider )
	{
		...
		// read in world data
		WorldFinalValues values = storage.getMetadataService().readWorldData();
		if ( values != null )
		{
		...
		functions = storage.getFunctionIoService().readFunctions().stream().collect( Collectors.toMap( CommandFunction::getFullName, function -> function ) );
	...
	{
		Bukkit.getServer().getScheduler().runTaskAsynchronously( null, () -> cb.onLoad( chunkManager.getChunk( x, z ) ) );
	}
	...
	{
		return entityManager.getAll().stream().filter( e -> cls.isAssignableFrom( e.getClass() ) ).map( e -> ( T ) e ).collect( Collectors.toCollection( ArrayList::new ) );
	}
	...
	{
		return entityManager.getAll().stream().filter( e -> e instanceof GlowLivingEntity ).map( e -> ( GlowLivingEntity ) e ).collect( Collectors.toCollection( LinkedList::new ) );
	}
	...
	private void handleSleepAndWake( List<GlowPlayer> players )
	{
		...
		if ( !players.isEmpty() )
		{
			...
			{ // otherwise check whether everyone is asleep
				boolean skipNight = gameRuleMap.getBoolean( "doDaylightCycle" ) && areAllPlayersSleeping( players );
	...
	public boolean setGameRuleValue( String rule, String value )
	{
		...
		else if ( rule.equals( "reducedDebugInfo" ) )
		{
			...
			EntityStatusMessage message = new EntityStatusMessage( 0, gameRuleMap.getBoolean( "reducedDebugInfo" ) ? EntityStatusMessage.ENABLE_REDUCED_DEBUG_INFO : EntityStatusMessage.DISABLE_REDUCED_DEBUG_INFO );
			for ( GlowPlayer player : getRawPlayers() )
			{
				player.getSession().send( message );
			}
