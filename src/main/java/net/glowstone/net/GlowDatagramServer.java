...
public abstract class GlowDatagramServer extends GlowNetworkServer
{
	...
	public GlowDatagramServer( GlowServer server, CountDownLatch latch )
	{
		...
		group = epoll ? new EpollEventLoopGroup() : kqueue ? new KQueueEventLoopGroup() : new NioEventLoopGroup();
		bootstrap = new Bootstrap();

		bootstrap.group( group ).channel( epoll ? EpollDatagramChannel.class : kqueue ? KQueueDatagramChannel.class : NioDatagramChannel.class ).option( ChannelOption.SO_KEEPALIVE, true );
	}
	...
	public void shutdown()
	{
		...
		{
			GlowServer.logger.log( Level.SEVERE, "Datagram server shutdown process interrupted!", e );
		}
	}
}
