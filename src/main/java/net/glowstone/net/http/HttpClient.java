...
{
	// private static DnsAddressResolverGroup resolverGroup = new DnsAddressResolverGroup( GlowServer.EPOLL ? EpollDatagramChannel.class : GlowServer.KQUEUE ? KQueueDatagramChannel.class : NioDatagramChannel.class, DefaultDnsServerAddressStreamProvider.INSTANCE );
	...
	public static void connect( String url, EventLoop eventLoop, HttpCallback callback )
	{
		...
		new Bootstrap().group( eventLoop ).channel( GlowServer.EPOLL ? EpollSocketChannel.class : GlowServer.KQUEUE ? KQueueSocketChannel.class : NioSocketChannel.class ).handler( new HttpChannelInitializer( sslCtx, callback ) ).option( ChannelOption.CONNECT_TIMEOUT_MILLIS, 5000 ).connect( InetSocketAddress.createUnresolved( host, port ) ).addListener( ( ChannelFutureListener ) future -> {
			if ( future.isSuccess() )
			{
