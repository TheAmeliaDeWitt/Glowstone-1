...
public final class CraftingManager implements Iterable<Recipe>
{
				...
				for ( int colStart = 0; colStart <= size - cols; ++colStart )
				{
					...
					for ( int row = 0; row < size; row++ )
					{
						for ( int col = 0; col < size; col++ )
						{
							...
							if ( ( row < rowStart || row >= rowStart + rows || col < colStart || col >= colStart + cols ) && items[row * size + col] != null )
							{
								continue position;
							}
						}
	...
	public void initialize()
	{
		...
		// Report stats
		GlowServer.logger.info( "Recipes: " + shapedRecipes.size() + " shaped, " + shapelessRecipes.size() + " shapeless, " + furnaceRecipes.size() + " furnace, " + dynamicRecipes.size() + " dynamic, " + furnaceFuels.size() + " fuels." );
	}
