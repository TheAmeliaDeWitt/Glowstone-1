...
public final class GlowMapRenderer extends MapRenderer
{
	...
	private static byte colorFor( Block block, int worldX, int worldZ )
	{
		...
		materialValues = block instanceof GlowBlock ? ( ( GlowBlock ) block ).getMaterialValues() : ( ( GlowServer ) Bukkit.getServer() ).getMaterialValueManager().getValues( block.getType() );
	...
	{
		return ( byte ) ( ( ( ( worldX * worldX * 0x4c1906 ) + ( worldX * 0x5ac0db ) + ( ( worldZ * worldZ ) * 0x4307a7 ) + ( worldZ * 0x5f24f ) ) ) % 4 );
	}
	...
	public void render( MapView map, MapCanvas canvas, Player player )
	{
		...
		for ( int pixelX = 0; pixelX < MAP_SIZE; pixelX++ )
		{
			for ( int pixelY = 0; pixelY < MAP_SIZE; pixelY++ )
			{
				...
				if ( ( ( worldX - playerX ) * ( worldX - playerX ) + ( worldZ - playerZ ) * ( worldZ - playerZ ) ) < MAP_SIGHT_DISTANCE_SQUARED )
				{
					// TODO: Should the highest block be skipped over if it's e.g. a flower or a
					// technical block?
					byte blockColor = colorFor( world.getHighestBlockAt( worldX, worldZ ), worldX, worldZ );
