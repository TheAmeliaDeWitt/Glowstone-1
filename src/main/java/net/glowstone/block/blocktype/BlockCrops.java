...
public class BlockCrops extends BlockNeedsAttached implements IBlockGrowable
{
	...
	public Collection<ItemStack> getDrops( GlowBlock block, ItemStack tool )
	{
		...
		{
			return Collections.unmodifiableList( Arrays.asList( new ItemStack( Material.SEEDS, ThreadLocalRandom.current().nextInt( 4 ) ), new ItemStack( Material.WHEAT, 1 ) ) );
		}
		else
		{
	...
	public void grow( GlowPlayer player, GlowBlock block )
	{
		...
		int cropState = block.getData() + ThreadLocalRandom.current().nextInt( CropState.MEDIUM.ordinal() ) + CropState.VERY_SMALL.ordinal();
	...
	public void updateBlock( GlowBlock block )
	{
		...
		if ( cropState < CropState.RIPE.ordinal() && block.getRelative( BlockFace.UP ).getLightLevel() >= 9 && ThreadLocalRandom.current().nextInt( ( int ) ( 25.0F / getGrowthRateModifier( block ) ) + 1 ) == 0 )
		{
			cropState++;
			if ( cropState > CropState.RIPE.ordinal() )
			{
