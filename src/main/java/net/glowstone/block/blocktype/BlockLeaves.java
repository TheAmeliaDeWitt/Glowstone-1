...
public class BlockLeaves extends BlockType
{
	...
	public Collection<ItemStack> getDrops( GlowBlock block, ItemStack tool )
	{
		...
		{
			return Collections.unmodifiableList( Arrays.asList( new ItemStack( block.getType(), 1, ( short ) data ) ) );
		}
		...
		if ( ThreadLocalRandom.current().nextFloat() < ( block.getData() == 3 ? .025f : .05f ) )
		{ //jungle leaves drop with 2.5% chance, others drop with 5%
			drops.add( new ItemStack( Material.SAPLING, 1, ( short ) data ) );
		}
		if ( data == 0 && ThreadLocalRandom.current().nextFloat() < .005 )
		{ //oak leaves have a .5% chance to drop an apple
