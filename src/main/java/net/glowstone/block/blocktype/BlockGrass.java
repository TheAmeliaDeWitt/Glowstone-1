	...
	public void updateBlock( GlowBlock block )
	{
		...
		else if ( blockAbove.getLightLevel() >= 9 )
		{
			...
			for ( int i = 0; i < 4; i++ )
			{
				...
				if ( targetBlock.getChunk().isLoaded() && targetAbove.getChunk().isLoaded() && targetBlock.getType() == Material.DIRT && targetBlock.getData() == 0 // only spread on normal dirt
						&& targetAbove.getMaterialValues().getLightOpacity() <= 2 && targetAbove.getLightLevel() >= 4 )
				{
