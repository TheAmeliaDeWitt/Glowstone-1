...
public class BlockSlab extends BlockType
{
	...
	{
		if ( block.getType() == Material.WOOD_STEP || tool != null && ToolType.PICKAXE.matches( tool.getType() ) )
		{
	...
	private boolean matchingType( GlowBlock block, BlockFace face, ItemStack holding, boolean ignoreFace )
	{
		...
		return ( blockType == Material.STEP || blockType == Material.WOOD_STEP || blockType == Material.STONE_SLAB2 || blockType == Material.PURPUR_SLAB ) && blockType == holding.getType() && ( face == BlockFace.UP && blockData == holdingData || face == BlockFace.DOWN && blockData - 8 == holdingData || ignoreFace && blockData % 8 == holdingData );
	}
