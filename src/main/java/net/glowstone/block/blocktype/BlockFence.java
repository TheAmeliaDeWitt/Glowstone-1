...
public class BlockFence extends BlockDirectDrops
{
	...
	public boolean blockInteract( GlowPlayer player, GlowBlock block, BlockFace face, Vector clickedLoc )
	{
		...
		if ( !player.getLeashedEntities().isEmpty() )
		{
			...
			ImmutableList.copyOf( player.getLeashedEntities() ).stream().filter( e -> !EventFactory.callEvent( new PlayerLeashEntityEvent( e, leashHitch, player ) ).isCancelled() ).forEach( e -> e.setLeashHolder( leashHitch ) );
			return true;
		}
		return false;
	}
