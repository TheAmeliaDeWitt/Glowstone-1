...
public class ItemBow extends ItemTimedUsage
{
	...
	public void endUse( GlowPlayer player, ItemStack bow )
	{
		...
		if ( launchedArrow != null )
		{
			...
			double chargeFraction = Math.max( 0.0, 1.0 - ( TICKS_TO_FULLY_CHARGE - player.getUsageTime() ) / TICKS_TO_FULLY_CHARGE );
			double damage = MAX_BASE_DAMAGE * chargeFraction * ( 1 + 0.25 * bow.getEnchantmentLevel( Enchantment.ARROW_DAMAGE ) );
			launchedArrow.setVelocity( launchedArrow.getVelocity().normalize().multiply( chargeFraction * MAX_SPEED ) );
