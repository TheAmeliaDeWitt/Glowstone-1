...
public class RectangularRegion
{
	...
	{
		Preconditions.checkArgument( from.getWorld() == to.getWorld(), "The given locations do not have matching worlds." );
		this.lowCorner = new Location( from.getWorld(), Double.min( from.getX(), to.getX() ), Double.min( from.getY(), to.getY() ), Double.min( from.getZ(), to.getZ() ) );
		this.highCorner = new Location( from.getWorld(), Double.max( from.getX(), to.getX() ), Double.max( from.getY(), to.getY() ), Double.max( from.getZ(), to.getZ() ) );
	 ...
	 */
	public Iterable<Location> blockLocations( IterationDirection directionX, IterationDirection directionY, IterationDirection directionZ )
	{
		return new LocationIterable( lowCorner, directionX.iterable( widthX ), directionY.iterable( widthY ), directionZ.iterable( widthZ ) );
	}
	...
	private static class LocationIterable implements Iterable<Location>
	{
		...
		private LocationIterable( Location lowCorner, Iterable<Integer> iterableX, Iterable<Integer> iterableY, Iterable<Integer> iterableZ )
		{
	...
	private static class LocationIterator extends AbstractIterator<Location>
	{
		...
		private LocationIterator( Location lowCorner, Iterable<Integer> iterableX, Iterable<Integer> iterableY, Iterable<Integer> iterableZ )
		{
			this.lowCorner = lowCorner;
		...
		{
			return new Location( lowCorner.getWorld(), lowCorner.getBlockX() + x, lowCorner.getBlockY() + y, lowCorner.getBlockZ() + iteratorZ.next() );
		}
	}
}
