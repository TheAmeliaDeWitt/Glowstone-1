...
public final class ListTag<T extends Tag> extends Tag<List<T>>
{
	...
	public ListTag( TagType childType, List<T> value )
	{
		...
		for ( Tag elem : value )
		{
			...
			{
				throw new IllegalArgumentException( "ListTag(" + childType + ") cannot hold tags of type " + elem.getType() );
			}
		}
	}
	...
	{
		builder.append( value.size() ).append( " entries of type " ).append( childType.getName() ).append( "\n{\n" );
		for ( T elem : value )
		{
