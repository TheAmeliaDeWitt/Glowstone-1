...
public class GlowPlayer extends GlowHumanEntity implements Player
{
	...
	public void pulse()
	{
		...
		if ( exhaustion > 4.0f )
		{
			...
			{
				FoodLevelChangeEvent event = EventFactory.callEvent( new FoodLevelChangeEvent( this, Math.max( foodLevel - 1, 0 ) ) );
				if ( !event.isCancelled() )
				{
	...
	public void sendMap( MapView map )
	{
		...
		session.send( new MapDataMessage( map.getId(), map.getScale().ordinal(), Collections.emptyList(), mapCanvas.toSection() ) );
	}
	...
	public void stopSound( String sound, SoundCategory category )
	{
		...
		try
		{
			...
			session.sendAndRelease( new PluginMessage( "MC|StopSound", buffer.array() ), buffer );
		}
		catch ( IOException e )
		{
	...
	private void streamBlocks()
	{
		...
		newChunks.stream().map( key -> world.getChunkAt( key.getX(), key.getZ() ).toMessage( skylight ) ).forEach( session::send );
