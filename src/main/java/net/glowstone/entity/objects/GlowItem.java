...
public class GlowItem extends GlowEntity implements Item
{
	...
	{
		return Arrays.asList( new SpawnObjectMessage( entityId, getUniqueId(), SpawnObjectMessage.ITEM, location ), new EntityMetadataMessage( entityId, metadata.getEntryList() ),
				// these keep the client from assigning a random velocity
				new EntityTeleportMessage( entityId, location ), new EntityVelocityMessage( entityId, getVelocity() ) );
	}
	...
	private boolean getPickedUp( GlowPlayer player )
	{
		...
		player.updateInventory(); // workaround for player editing slot & it immediately being
		// filled again
		if ( !map.isEmpty() )
		{
