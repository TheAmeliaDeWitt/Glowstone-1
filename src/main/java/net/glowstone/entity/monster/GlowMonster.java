...
public class GlowMonster extends GlowCreature implements Monster
{
	...
	public boolean canDespawnImmediately()
	{
		...
		boolean hasNearbyPlayer = world.getPlayers().stream().anyMatch( player -> location.distanceSquared( player.getLocation() ) <= DESPAWN_RANGE_IMMEDIATE * DESPAWN_RANGE_IMMEDIATE );
		if ( hasNearbyPlayer )
		{
			return false;
		}
