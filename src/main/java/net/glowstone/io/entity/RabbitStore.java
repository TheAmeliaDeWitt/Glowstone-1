...
{
	private final Map<Type, Integer> rabbitTypeIntegerMap = ImmutableMap.<Type, Integer>builder().put( Type.BROWN, 0 ).put( Type.WHITE, 1 ).put( Type.BLACK, 2 ).put( Type.BLACK_AND_WHITE, 3 ).put( Type.GOLD, 4 ).put( Type.SALT_AND_PEPPER, 5 ).put( Type.THE_KILLER_BUNNY, 99 ).build();
	private final Map<Integer, Type> rabbitTypeMap = ImmutableMap.<Integer, Type>builder().put( 0, Type.BROWN ).put( 1, Type.WHITE ).put( 2, Type.BLACK ).put( 3, Type.BLACK_AND_WHITE ).put( 4, Type.GOLD ).put( 5, Type.SALT_AND_PEPPER ).put( 99, Type.THE_KILLER_BUNNY ).build();
	...
	public void load( GlowRabbit entity, CompoundTag compound )
	{
		...
		if ( compound.isInt( "RabbitType" ) )
		{
			...
			rabbitType = rabbitTypeMap.getOrDefault( rabbitId, Type.BROWN );
