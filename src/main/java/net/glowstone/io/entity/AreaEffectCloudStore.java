...
class AreaEffectCloudStore extends EntityStore<GlowAreaEffectCloud>
{
	...
	public void load( GlowAreaEffectCloud entity, CompoundTag tag )
	{
		...
		if ( tag.isString( PARTICLE ) )
		{
			...
			{
				GlowServer.logger.warning( () -> String.format( "Ignoring invalid particle type %s in tag %s", particle, tag ) );
			}
		}
		// TODO: Potion
		if ( tag.isCompoundList( EFFECTS ) )
		{
			tag.getCompoundList( EFFECTS ).stream().map( GlowMetaPotion::fromNbt ).forEach( effect -> entity.addCustomEffect( effect, false ) );
		}
	}
