...
public class Lake implements TerrainObject
{
	...
	private static final SortedSet<Material> PLANT_TYPES = ImmutableSortedSet.of( Material.LONG_GRASS, Material.YELLOW_FLOWER, Material.RED_ROSE, Material.DOUBLE_PLANT, Material.BROWN_MUSHROOM, Material.RED_MUSHROOM );
	...
	private boolean canPlace( byte[] lakeMap, World world, int sourceX, int sourceY, int sourceZ )
	{
		for ( int x = 0; x < MAX_DIAMETER; x++ )
		{
			for ( int z = 0; z < MAX_DIAMETER; z++ )
			{
				...
				{
					if ( isLakeBlock( lakeMap, x, y, z ) || ( ( ( x >= ( MAX_DIAMETER - 1 ) ) || !isLakeBlock( lakeMap, x + 1, y, z ) ) && ( ( x <= 0 ) || !isLakeBlock( lakeMap, x - 1, y, z ) ) && ( ( z >= ( MAX_DIAMETER - 1 ) ) || !isLakeBlock( lakeMap, x, y, z + 1 ) ) && ( ( z <= 0 ) || !isLakeBlock( lakeMap, x, y, z - 1 ) ) && ( ( z >= ( MAX_HEIGHT - 1 ) ) || !isLakeBlock( lakeMap, x, y + 1, z ) ) && ( ( z <= 0 ) || !isLakeBlock( lakeMap, x, y - 1, z ) ) ) )
					{
						continue;
					}
		...
		for ( int x = 0; x < ( int ) MAX_DIAMETER; x++ )
		{
			for ( int z = 0; z < ( int ) MAX_DIAMETER; z++ )
			{
				for ( int y = 0; y < ( int ) MAX_HEIGHT; y++ )
				{
					...
					if ( blockType == Material.DIRT && ( blockAboveType == Material.LOG || blockAboveType == Material.LOG_2 ) || blockType == Material.LOG || blockType == Material.LOG_2 )
					{
						continue;
					}
					if ( y >= ( int ) MAX_HEIGHT / 2 )
					{
						...
						if ( PLANT_TYPES.contains( blockAboveType ) )
						{
							if ( blockAboveType == Material.DOUBLE_PLANT )
							{
								...
								MaterialData blockAboveData = blockAboveBlock.getState().getData();
								if ( blockAboveData instanceof DoublePlant && ( ( DoublePlant ) blockAboveData ).getSpecies() == DoublePlantSpecies.PLANT_APEX )
								{
						...
						if ( this.type == Material.STATIONARY_WATER && ( blockType == Material.ICE || blockType == Material.PACKED_ICE ) )
						{
							type = blockType;
						}
					}
		...
		for ( int x = 0; x < ( int ) MAX_DIAMETER; x++ )
		{
			for ( int z = 0; z < ( int ) MAX_DIAMETER; z++ )
			{
				for ( int y = ( int ) MAX_HEIGHT / 2; y < ( int ) MAX_HEIGHT; y++ )
				{
					...
					Block blockAbove = block.getRelative( BlockFace.UP );
					if ( block.getType() == Material.DIRT && !blockAbove.getType().isOccluding() && blockAbove.getLightLevel() > 0 )
					{
