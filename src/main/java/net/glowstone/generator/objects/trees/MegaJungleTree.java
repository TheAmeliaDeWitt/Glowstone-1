...
public class MegaJungleTree extends GenericTree
{
	...
	private void addVinesOnTrunk( World world, int blockX, int blockY, int blockZ, Random random )
	{
		...
		{
			maybePlaceVine( world, blockX + -1, blockY + y, blockZ + 0, BlockFace.EAST, random );
			maybePlaceVine( world, blockX + 0, blockY + y, blockZ + -1, BlockFace.SOUTH, random );
			maybePlaceVine( world, blockX + 2, blockY + y, blockZ + 0, BlockFace.WEST, random );
			maybePlaceVine( world, blockX + 1, blockY + y, blockZ + -1, BlockFace.SOUTH, random );
			maybePlaceVine( world, blockX + 2, blockY + y, blockZ + 1, BlockFace.WEST, random );
			maybePlaceVine( world, blockX + 1, blockY + y, blockZ + 2, BlockFace.NORTH, random );
			maybePlaceVine( world, blockX + -1, blockY + y, blockZ + 1, BlockFace.EAST, random );
			maybePlaceVine( world, blockX + 0, blockY + y, blockZ + 2, BlockFace.NORTH, random );
		}
	}
	...
	public boolean generate( World world, Random random, int blockX, int blockY, int blockZ )
	{
		...
		for ( int y = -2; y <= 0; y++ )
		{
			generateLeaves( blockX + 0, blockY + height + y, blockZ, 3 - y, false, world );
		}

		// generates the branches
		int branchHeight = height - 2 - random.nextInt( 4 );
		while ( branchHeight > height / 2 )
		{ // branching start at least at middle height
			int x = 0;
			int z = 0;
			// generates a branch
			...
			for ( int i = 0; i < 5; i++ )
			{
				...
				delegate.setTypeAndRawData( world, blockX + x, blockY + branchHeight - 3 + i / 2, blockZ + z, Material.LOG, logType );
			}
			// generates leaves for this branch
			for ( int y = branchHeight - ( random.nextInt( 2 ) + 1 ); y <= branchHeight; y++ )
			{
				generateLeaves( blockX + x, blockY + y, blockZ + z, 1 - ( y - branchHeight ), true, world );
			}
	...
	protected void generateDirtBelowTrunk( World world, int blockX, int blockY, int blockZ )
	{
		...
		delegate.setTypeAndData( world, blockX + 0, blockY + -1, blockZ, Material.DIRT, dirt );
		delegate.setTypeAndData( world, blockX + 0, blockY + -1, blockZ + 1, Material.DIRT, dirt );
		delegate.setTypeAndData( world, blockX + 1, blockY + -1, blockZ, Material.DIRT, dirt );
		delegate.setTypeAndData( world, blockX + 1, blockY + -1, blockZ + 1, Material.DIRT, dirt );
	}

	protected void generateLeaves( int sourceX, int sourceY, int sourceZ, int radius, boolean odd, World world )
	{
		int n = 1;
		if ( odd )
		{
	...
	protected void generateTrunk( World world, int blockX, int blockY, int blockZ )
	{
		...
		{
			Material type = world.getBlockAt( blockX + 0, blockY + y, blockZ + 0 ).getType();
			if ( type == Material.AIR || type == Material.LEAVES )
			{
				delegate.setTypeAndRawData( world, blockX + 0, blockY + y, blockZ, Material.LOG, logType );
			}
			type = world.getBlockAt( blockX + 0, blockY + y, blockZ + 1 ).getType();
			if ( type == Material.AIR || type == Material.LEAVES )
			{
				delegate.setTypeAndRawData( world, blockX + 0, blockY + y, blockZ + 1, Material.LOG, logType );
			}
			type = world.getBlockAt( blockX + 1, blockY + y, blockZ + 0 ).getType();
			if ( type == Material.AIR || type == Material.LEAVES )
			{
				delegate.setTypeAndRawData( world, blockX + 1, blockY + y, blockZ, Material.LOG, logType );
			}
			type = world.getBlockAt( blockX + 1, blockY + y, blockZ + 1 ).getType();
			if ( type == Material.AIR || type == Material.LEAVES )
			{
				delegate.setTypeAndRawData( world, blockX + 1, blockY + y, blockZ + 1, Material.LOG, logType );
			}
		}
	}

	private void maybePlaceVine( World world, int absoluteX, int absoluteY, int absoluteZ, BlockFace facingDirection, Random random )
	{
		if ( random.nextInt( 3 ) != 0 && blockTypeAt( absoluteX, absoluteY, absoluteZ, world ) == Material.AIR )
		{
			delegate.setTypeAndData( world, absoluteX, absoluteY, absoluteZ, Material.VINE, new Vine( facingDirection ) );
		}
	}
