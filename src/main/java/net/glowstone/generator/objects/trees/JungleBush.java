...
public class JungleBush extends GenericTree
{
	...
	public boolean generate( World world, Random random, int blockX, int blockY, int blockZ )
	{
		...
		while ( ( l.getBlock().getType() == Material.AIR || l.getBlock().getType() == Material.LEAVES ) && blockY > 0 )
		{
			l.subtract( 0, 1, 0 );
		}
		...
		delegate.setTypeAndRawData( world, blockX, adjustedY + 1, blockZ, Material.LOG, logType );
		...
		for ( int y = adjustedY + 1; y <= adjustedY + 3; y++ )
		{
			...
			for ( int x = blockX - radius; x <= blockX + radius; x++ )
			{
				...
				{
					if ( ( Math.abs( x - l.getBlockX() ) != radius || Math.abs( z - l.getBlockZ() ) != radius || random.nextBoolean() ) && !delegate.getBlockState( world, x, y, z ).getType().isSolid() )
					{
