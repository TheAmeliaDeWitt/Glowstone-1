...
public class DarkOakTree extends GenericTree
{
	...
	public DarkOakTree( Random random, BlockStateDelegate delegate )
	{
		...
		setOverridables( Material.AIR, Material.LEAVES, Material.LEAVES_2, Material.GRASS, Material.DIRT, Material.LOG, Material.LOG_2, Material.SAPLING, Material.VINE );
	...
	public boolean generate( World world, Random random, int blockX, int blockY, int blockZ )
	{
		...
		for ( int y = 0; y < height; y++ )
		{
			...
			if ( material == Material.AIR || material == Material.LEAVES )
			{
				...
				delegate.setTypeAndRawData( world, centerX, blockY + y, centerZ, Material.LOG_2, 1 );
				delegate.setTypeAndRawData( world, centerX, blockY + y, centerZ + 1, Material.LOG_2, 1 );
				delegate.setTypeAndRawData( world, centerX + 1, blockY + y, centerZ, Material.LOG_2, 1 );
				delegate.setTypeAndRawData( world, centerX + 1, blockY + y, centerZ + 1, Material.LOG_2, 1 );
			}
		}
		...
		for ( int x = -1; x <= 2; x++ )
		{
			for ( int z = -1; z <= 2; z++ )
			{
				if ( ( x == -1 || z == -1 || x == 2 || z == 2 ) && random.nextInt( 3 ) == 0 )
				{
					...
					{
						Material material = blockTypeAt( blockX + x, trunkTopY - y - 1, blockZ + z, world );
						if ( material == Material.AIR || material == Material.LEAVES )
						{
							delegate.setTypeAndRawData( world, blockX + x, trunkTopY - y - 1, blockZ + z, Material.LOG_2, 1 );
						}
					}
		...
		delegate.setTypeAndData( world, blockX, blockY - 1, blockZ + 1, Material.DIRT, dirt );
