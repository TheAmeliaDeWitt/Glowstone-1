...
public class BrownMushroomTree extends GenericTree
{
	...
	public BrownMushroomTree( Random random, BlockStateDelegate delegate )
	{
		...
		setOverridables( Material.AIR, Material.LEAVES, Material.LEAVES_2 );
	...
	{
		return soil.getType() == Material.GRASS || soil.getType() == Material.DIRT || soil.getType() == Material.MYCEL;
	}
	...
	public boolean generate( World world, Random random, int blockX, int blockY, int blockZ )
	{
		...
		{
			delegate.setTypeAndRawData( world, blockX, blockY + y, blockZ, type, 10 ); // stem texture
		}
		...
		for ( int y = capY; y <= blockY + height; y++ )
		{ // from bottom to top of mushroom
			...
			for ( int x = blockX - radius; x <= blockX + radius; x++ )
			{
				for ( int z = blockZ - radius; z <= blockZ + radius; z++ )
				{
					...
					if ( type == Material.HUGE_MUSHROOM_1 || y < blockY + height )
					{
						...
						if ( ( x == blockX - radius || x == blockX + radius ) && ( z == blockZ - radius || z == blockZ + radius ) )
						{
							continue;
						}
						...
						else if ( x == blockX - radius && z == blockZ - ( radius - 1 ) )
						{
							data = 1; // cap texture on top, west and north
						}
						else if ( x == blockX + radius - 1 && z == blockZ - radius )
						{
