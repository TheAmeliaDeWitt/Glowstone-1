...
public class CocoaTree extends JungleTree
{
			...
			for ( int y = 0; y < 2; y++ )
			{
				...
				{ // rotate the 4 trunk faces
					...
					{ // higher it is, more chances there is
						...
						Block block = delegate.getBlockState( world, sourceX, sourceY + height - 5 + y, sourceZ ).getBlock().getRelative( cocoaFace );
						delegate.setTypeAndData( world, block.getX(), block.getY(), block.getZ(), Material.COCOA, new CocoaPlant( size, cocoaFace.getOppositeFace() ) );
					}
				}
			}
		}
	...
	protected void addVinesOnLeaves( int baseX, int baseY, int baseZ, World world, Random random )
	{
		for ( int y = baseY - 3 + height; y <= baseY + height; y++ )
		{
			...
			for ( int x = baseX - radius; x <= baseX + radius; x++ )
			{
				...
				{
					if ( blockTypeAt( x, y, z, world ) == Material.LEAVES )
					{
						if ( random.nextInt( 4 ) == 0 && blockTypeAt( x - 1, y, z, world ) == Material.AIR )
						{
						...
						if ( random.nextInt( 4 ) == 0 && blockTypeAt( x, y, z - 1, world ) == Material.AIR )
						{
	...
	private void addVinesOnTrunk( int trunkX, int trunkY, int trunkZ, World world, Random random )
	{
		...
		{
			if ( random.nextInt( 3 ) != 0 && blockTypeAt( trunkX - 1, trunkY + y, trunkZ, world ) == Material.AIR )
			{
				delegate.setTypeAndData( world, trunkX - 1, trunkY + y, trunkZ, Material.VINE, new Vine( BlockFace.EAST ) );
			}
			if ( random.nextInt( 3 ) != 0 && blockTypeAt( trunkX + 1, trunkY + y, trunkZ, world ) == Material.AIR )
			{
				delegate.setTypeAndData( world, trunkX + 1, trunkY + y, trunkZ, Material.VINE, new Vine( BlockFace.WEST ) );
			}
			if ( random.nextInt( 3 ) != 0 && blockTypeAt( trunkX, trunkY + y, trunkZ - 1, world ) == Material.AIR )
			{
				delegate.setTypeAndData( world, trunkX, trunkY + y, trunkZ - 1, Material.VINE, new Vine( BlockFace.SOUTH ) );
			}
			if ( random.nextInt( 3 ) != 0 && blockTypeAt( trunkX, trunkY + y, trunkZ + 1, world ) == Material.AIR )
			{
				delegate.setTypeAndData( world, trunkX, trunkY + y, trunkZ + 1, Material.VINE, new Vine( BlockFace.NORTH ) );
			}
		}
	}
