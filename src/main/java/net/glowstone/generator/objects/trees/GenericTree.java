...
public class GenericTree implements TerrainObject
{
	...
	public GenericTree( Random random, BlockStateDelegate delegate )
	{
		...
		setOverridables( Material.AIR, Material.LEAVES, Material.GRASS, Material.DIRT, Material.LOG, Material.LOG_2, Material.SAPLING, Material.VINE );
	...
	{
		return soil.getType() == Material.GRASS || soil.getType() == Material.DIRT || soil.getType() == Material.SOIL;
	}
	 ...
	 */
	protected boolean cannotGenerateAt( int baseX, int baseY, int baseZ, World world )
	{
		return !canHeightFit( baseY ) || !canPlaceOn( world.getBlockAt( baseX, baseY - 1, baseZ ).getState() ) || !canPlace( baseX, baseY, baseZ, world );
	}
	...
	{
		return generate( loc.getWorld(), ThreadLocalRandom.current(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ() );
	}
	...
	public boolean generate( World world, Random random, int blockX, int blockY, int blockZ )
	{
		...
		for ( int y = blockY + height - 3; y <= blockY + height; y++ )
		{
			...
			for ( int x = blockX - radius; x <= blockX + radius; x++ )
			{
				...
				{
					if ( Math.abs( x - blockX ) != radius || Math.abs( z - blockZ ) != radius || random.nextBoolean() && n != 0 )
					{
		...
		{
			replaceIfAirOrLeaves( blockX, blockY + y, blockZ, Material.LOG, logType, world );
		}
	 ...
	 * @param world       the world we are generating in
	 */
	protected void replaceIfAirOrLeaves( int x, int y, int z, Material newMaterial, int data, World world )
	{
