...
public class BigOakTree extends GenericTree
{
	...
	private int countAvailableBlocks( Vector from, Vector to, World world )
	{
		...
		for ( int i = 0; i <= maxDistance; i++, n++ )
		{
			...
			if ( target.getBlockY() < 0 || target.getBlockY() > 255 || !overridables.contains( blockTypeAt( target.getBlockX(), target.getBlockY(), target.getBlockZ(), world ) ) )
			{
				return n;
			}
		}
	...
	{
		if ( !canPlaceOn( world.getBlockAt( blockX, blockY - 1, blockZ ).getState() ) || !canPlace( blockX, blockY, blockZ, world ) )
		{
			return false;
		}
			...
			for ( int y = 0; y < maxLeafDistance; y++ )
			{
				...
				for ( int x = -nodeDistance; x <= nodeDistance; x++ )
				{
					for ( int z = -nodeDistance; z <= nodeDistance; z++ )
					{
						...
						{
							if ( overridables.contains( blockTypeAt( node.getX() + x, node.getY() + y, node.getZ() + z, world ) ) )
							{
								delegate.setTypeAndRawData( world, node.getX() + x, node.getY() + y, node.getZ() + z, Material.LEAVES, leavesType );
							}
						}
					}
				}
		...
		{
			delegate.setTypeAndRawData( world, blockX, blockY + y, blockZ, Material.LOG, logType );
		}
		...
		for ( LeafNode node : leafNodes )
		{
			if ( ( double ) node.getBranchY() - blockY >= height * 0.2D )
			{
				...
				{
					branch = base.clone().add( new Vector( 0.5 + i * dx, 0.5 + i * dy, 0.5 + i * dz ) );
					...
					delegate.setTypeAndRawData( world, branch.getBlockX(), branch.getBlockY(), branch.getBlockZ(), Material.LOG, logType | direction );
				}
			}
		}
	...
	private Collection<LeafNode> generateLeafNodes( int blockX, int blockY, int blockZ, World world, Random random )
	{
		...
		for ( int l = --y - blockY; l >= 0; l--, y-- )
		{
			float h = height / 2.0F;
			float v = h - l;
			float f = l < ( float ) height * 0.3D ? -1.0F : v == h ? h * 0.5F : h <= Math.abs( v ) ? 0.0F : ( float ) Math.sqrt( h * h - v * v ) * 0.5F;
			if ( f >= 0.0F )
			{
				for ( int i = 0; i < nodeCount; i++ )
				{
					...
					if ( countAvailableBlocks( new Vector( x, y, z ), new Vector( x, y + maxLeafDistance, z ), world ) == -1 )
					{
						int offX = blockX - x;
						int offZ = blockZ - z;
						double distance = 0.381D * Math.sqrt( offX * offX + offZ * offZ );
						int branchBaseY = Math.min( trunkTopY, ( int ) ( y - distance ) );
						if ( countAvailableBlocks( new Vector( x, branchBaseY, z ), new Vector( x, y, z ), world ) == -1 )
						{
