...
public class BlockPatch implements TerrainObject
{
	...
	private static final List<Material> PLANT_TYPES = ImmutableList.of( Material.LONG_GRASS, Material.YELLOW_FLOWER, Material.RED_ROSE, Material.DOUBLE_PLANT, Material.BROWN_MUSHROOM, Material.RED_MUSHROOM );
						...
						if ( overridables.contains( block.getType() ) )
						{
							...
							if ( PLANT_TYPES.contains( mat ) )
							{
								if ( mat == Material.DOUBLE_PLANT )
								{
									...
									if ( data instanceof DoublePlant && ( ( DoublePlant ) data ).getSpecies() == DoublePlantSpecies.PLANT_APEX )
									{
