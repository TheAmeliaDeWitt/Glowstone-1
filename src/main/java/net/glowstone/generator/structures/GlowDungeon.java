...
public class GlowDungeon extends GlowStructurePiece
{
	...
	private final EntityType[] mobTypes = new EntityType[] {EntityType.SKELETON, EntityType.ZOMBIE, EntityType.ZOMBIE, EntityType.SPIDER};
		...
		for ( int x = 0; x < sizeX; x++ )
		{
			for ( int z = 0; z < sizeZ; z++ )
			{
				for ( int y = 0; y < HEIGHT; y++ )
				{
					...
					if ( ( x == 0 || x == sizeX - 1 || z == 0 || z == sizeZ - 1 ) && y == 1 && type == Material.AIR && builder.getBlockState( new Vector( x, y + 1, z ) ).getType() == Material.AIR )
					{
						i++;
