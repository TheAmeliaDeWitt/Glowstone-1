...
public class NetherGenerator extends GlowChunkGenerator
{
	...
	public ChunkData generateChunkData( World world, Random random, int chunkX, int chunkZ, BiomeGrid biomes )
	{
		...
		double[] surfaceNoise = ( ( PerlinOctaveGenerator ) getWorldOctaves( world ).get( "surface" ) ).getFractalBrownianMotion( cx, cz, 0, 0.5D, 2.0D );
		double[] soulsandNoise = ( ( PerlinOctaveGenerator ) getWorldOctaves( world ).get( "soulsand" ) ).getFractalBrownianMotion( cx, cz, 0, 0.5D, 2.0D );
		double[] gravelNoise = ( ( PerlinOctaveGenerator ) getWorldOctaves( world ).get( "gravel" ) ).getFractalBrownianMotion( cx, 0, cz, 0.5D, 2.0D );
		for ( int x = 0; x < 16; x++ )
		{
			for ( int z = 0; z < 16; z++ )
			{
